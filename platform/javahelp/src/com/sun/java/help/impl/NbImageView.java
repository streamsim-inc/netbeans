 ...
 * @since 1.4
 */
public class NbImageView extends View {
    /**
     ...
     */
    public NbImageView(Element elem) {
        super(elem);
    ...
    public void paint(Graphics g, Shape a) {
        ...
        final Image img = getImage(host == null || host.isEnabled());
        if (img != null) {
            if (! hasPixels(img)) {
                ...
                // Draw the image
                HiDPIUtils.paintAtScale1x(g,
                        rect.x + leftInset,
                        rect.y + topInset,
                        width, height,
                        new HiDPIUtils.HiDPIPainter() {
                    @Override
                    public void paint(Graphics2D g, int width, int height, double scale) {
                        Graphics2D g2 = (Graphics2D) g.create();

                        g2.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BICUBIC);
                        g2.drawImage(img, 0, 0, width, height, imageObserver);
